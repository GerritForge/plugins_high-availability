...
:   Maximum number of threads used to send batch index events to the target instance.
    Batch index events are internally generated by Gerrit and not associated to
    an interactive action performed by a user. Example of batch index event is the
    online reindexing. When set to zero, the batch index events are sent using the
    in the same thread pool of the interactive events.
    Defaults to 0.
